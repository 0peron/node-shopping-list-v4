...
app.put('/recipes/:id', jsonParser, (req, res) => {
    const input = ['name', 'ingredients', 'id'];
    for (let i = 0, i < input.length; i++) {
        const listData = input[i];
        if (!(field in req.body)) {
            const alert = `Missing\`${listData}\` in request body`
            console.error(alert);
            retun res.status(400).send(alert);
        }
        console.log(`updating recipe item \`${req.params.id}\``);
        const upItem = Recipes.update({
            id: req.params.id,
            name: req.body.name,
